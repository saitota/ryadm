...
fn precedence_higher_weight_condition_wins_on_disk() {
    ...
    assert!(tb
        .radm(&["config", "--add", "local.class", "winclass"])
        .success());

    tb.write_home("f1##default", "default-content\n");
    tb.write_home("f1##class.winclass", "class-content\n");
    assert!(tb
        .radm(&["add", "f1##default", "f1##class.winclass"])
        .success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn precedence_combined_conditions_beat_single_condition() {
    ...
    assert!(tb
        .radm(&["add", "f1##class.winclass", &combo_name])
        .success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn negated_condition_that_does_not_match_still_links() {
    ...
    assert!(tb.radm(&["add", "f1##~os.BogusOS"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn negated_condition_that_matches_aborts_and_never_links() {
    ...
    assert!(tb.radm(&["add", &name]).success());
    assert!(tb.radm(&["commit", "-m", "t2"]).success());
...
fn invalid_alt_warns_on_stderr_only() {
    ...
    assert!(tb.radm(&["add", "f1##invalid"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn valid_alts_produce_no_warning() {
    ...
    assert!(tb.radm(&["add", "f1##default"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn combined_conditions_all_must_match_to_link() {
    ...
    assert!(tb.radm(&["add", &good, &bad]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn directory_alt_links_contained_file_relative() {
    ...
    assert!(tb.radm(&["add", "mydir##default/inner"]).success());
    assert!(tb.radm(&["commit", "-m", "dir"]).success());
...
fn legacy_dir_symlink_is_replaced_by_per_file_symlinks() {
    ...
    assert!(tb.radm(&["add", "legacydir##default/inner"]).success());
    assert!(tb.radm(&["commit", "-m", "legacy"]).success());
...
fn alt_copy_true_copies_regular_file_not_symlink() {
    ...
    let os = host_os();
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.alt-copy", "true"]).success());
...
fn alt_copy_false_links_instead_of_copying() {
    ...
    let os = host_os();
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.alt-copy", "false"]).success());
...
fn alt_dir_source_maps_to_home_relative_target() {
    ...
    let r = tb.radm(&["add", ".config/yadm/alt/f1##default"]);
    assert!(r.success());
...
fn ensure_alt_path_creates_intermediate_dirs_symlink_style() {
    ...
    let r = tb.radm(&["add", ".config/yadm/alt/a/b/c/file##default"]);
    assert!(r.success());
    assert_eq!(r.stdout, "");
    assert_eq!(r.stderr, "");
...
fn ensure_alt_path_creates_intermediate_dirs_template_style() {
    ...
    let r = tb.radm(&["add", ".config/yadm/alt/a/b/c/file##template"]);
    assert!(r.success());
    assert_eq!(r.stdout, "");
    assert_eq!(r.stderr, "");
    assert_eq!(tb.read_home("a/b/c/file").trim(), "test-data");
...
fn alt_exclude_status_flag_default_and_true_are_ignored() {
    for autoexclude in [None, Some("true")] {
        ...
        if let Some(v) = autoexclude {
            assert!(tb.radm(&["config", "yadm.auto-exclude", v]).success());
        }

        tb.write_home("f1##default", "content\n");
        assert!(tb.radm(&["add", "f1##default"]).success());
        assert!(tb.radm(&["commit", "-m", "t"]).success());
        assert!(tb.radm(&["alt"]).success());

        let r = tb.radm(&["status", "-z", "-uall", "--ignored"]);
        assert!(r.success());
...
fn alt_exclude_status_flag_false_is_untracked_not_ignored() {
    ...
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb.radm(&["config", "yadm.auto-exclude", "false"]).success());
...
fn auto_alt_runs_quietly_on_status_when_explicitly_true() {
    ...
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.auto-alt", "true"]).success());
...
fn local_arch_override_affects_selection() {
    ...
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb.radm(&["config", "local.arch", "FakeArch"]).success());

    tb.write_home("f2##arch.FakeArch", "content-arch\n");
    assert!(tb.radm(&["add", "f2##arch.FakeArch"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn local_hostname_override_affects_selection() {
    ...
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb.radm(&["config", "local.hostname", "fakehost"]).success());

    tb.write_home("f1##hostname.fakehost", "content\n");
    assert!(tb.radm(&["add", "f1##hostname.fakehost"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn local_user_override_affects_selection() {
    ...
    assert!(tb.radm(&["init"]).success());
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb.radm(&["config", "local.user", "fakeuser"]).success());

    tb.write_home("f2##user.fakeuser", "content\n");
    assert!(tb.radm(&["add", "f2##user.fakeuser"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn class_matching_is_case_insensitive() {
    ...
    assert!(tb
        .radm(&["config", "--add", "local.class", "TestClass"])
        .success());

    tb.write_home("f1##class.testclass", "content\n");
    assert!(tb.radm(&["add", "f1##class.testclass"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());
...
fn multiple_local_class_values_all_participate_in_matching() {
    ...
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb
        .radm(&["config", "--add", "local.class", "before"])
        .success());
    assert!(tb
        .radm(&["config", "--add", "local.class", "testclass"])
        .success());
    assert!(tb
        .radm(&["config", "--add", "local.class", "after"])
        .success());
    ...
    let get_all = tb.radm(&["config", "--get-all", "local.class"]);
...
fn stale_link_removed_when_class_no_longer_matches() {
    ...
    assert!(tb.radm(&["config", "yadm.auto-alt", "false"]).success());
    assert!(tb
        .radm(&["config", "--add", "local.class", "staleclass"])
        .success());

    tb.write_home("stale##class.staleclass", "content\n");
    assert!(tb.radm(&["add", "stale##class.staleclass"]).success());
    assert!(tb.radm(&["commit", "-m", "t"]).success());

    assert!(tb.radm(&["alt"]).success());
    assert!(tb.is_symlink("stale"));

    assert!(tb.radm(&["config", "--unset-all", "local.class"]).success());
    assert!(tb
        .radm(&["config", "--add", "local.class", "somethingelse"])
        .success());
